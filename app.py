...
This dashboard analyzes the impact of sleep,
screen time, academic pressure and lifestyle
factors on student burnout and mental health.
""")
# =========================
# LOAD DATA & MODEL
# =========================
...
st.markdown("""
Analyze Student Burnout, Screen Time,
...
with col2:
    st.metric(
        "Avg Sleep",
        round(df["daily_sleep_hours"].mean(), 2)
    )

with col3:
    st.metric(
        "Avg Screen Time",
        round(df["screen_time_hours"].mean(), 2)
    )
...
if model_loaded:
    ...
    st.warning(
        "burnout_model.pkl not found. Train and save the model first."
    )
...
st.caption(
    "Created by Zubair Ansari | Data Science Portfolio Project 🚀"
)